...
let rpPicked=null,rpMode='both',rpChart=null;
...
function calcDinkRating(statsArr, sessions, players) {
  ...
  // Build per-player session avg pts for partner-adjustment lookup
  ...
  // Build partner map: for each player, average their partners' roundPts per shared game
...
// ── Round Progression chart helpers ──
function rpSetPlayer(id){rpPicked=id;rpChart&&rpChart.destroy();rpChart=null;setTimeout(rpRenderChart,10)}
function rpSetMode(m){rpMode=m;rpChart&&rpChart.destroy();rpChart=null;setTimeout(rpRenderChart,10)}
function rpRenderChart(){
  const canvas=document.getElementById('rpCanvas');if(!canvas||!window.Chart)return;
  const l=gL();const ss=gSS();const s=gS();if(!l)return;
  const sessions=ss?[ss]:(s?s.sessions:[]);
  const stats=calcStats(sessions,l.players).filter(x=>x.roundRes.length>0);
  if(!stats.length)return;
  const sorted=[...stats].sort((a,b)=>b.pf-a.pf);
  if(!rpPicked)rpPicked=sorted[0]?.id;
  const player=stats.find(x=>x.id===rpPicked);
  if(!player||!player.roundRes.length)return;
  // update chips
  const chEl=document.getElementById('rpChips');
  if(chEl)chEl.innerHTML=sorted.map(p=>'<button class="rp-chip'+(rpPicked===p.id?' on':'')+'" onclick="rpSetPlayer(\''+p.id+'\')">'+p.name.split(' ')[0]+'</button>').join('');
  // update mode buttons
  ['both','court','pts'].forEach(m=>{const b=document.getElementById('rpBtn-'+m);if(b){b.className='rp-mbtn'+(rpMode===m?' active-'+m:'')}});
  const refSS=ss||(s?.sessions?.slice().reverse().find(x=>x.started));
  const nC=refSS?.config?.courts||4;
  const courtLetters={};for(let i=1;i<=nC;i++)courtLetters[i]=String.fromCharCode(65+nC-i);
  const rounds=player.roundRes;
  const labels=rounds.map((_,i)=>'R'+(i+1));
  const courtData=rounds.map(r=>r.court);
  const ptsData=rounds.map(r=>r.pf);
  const datasets=[];
  if(rpMode==='court'||rpMode==='both'){
    datasets.push({label:'Court',data:courtData,yAxisID:'yCourt',borderColor:'#00e5ff',backgroundColor:'rgba(0,229,255,0.06)',borderWidth:2,pointBackgroundColor:rounds.map(r=>r.won?'#00e5ff':'rgba(255,92,71,0.7)'),pointBorderColor:'transparent',pointRadius:5,pointHoverRadius:7,tension:.3,fill:false,order:1});}
  if(rpMode==='pts'||rpMode==='both'){
    datasets.push({label:'Points',data:ptsData,yAxisID:'yPts',borderColor:'rgba(200,255,0,0.85)',backgroundColor:'rgba(200,255,0,0.04)',borderWidth:2,pointBackgroundColor:rounds.map(r=>r.won?'#c8ff00':'#ff5c47'),pointBorderColor:'transparent',pointRadius:5,pointHoverRadius:7,tension:.3,fill:false,borderDash:rpMode==='both'?[5,3]:[],order:2});}
  const scales={x:{ticks:{color:'rgba(255,255,255,0.25)',font:{size:10},maxRotation:0},grid:{color:'rgba(255,255,255,0.05)'},border:{display:false}}};
  if(rpMode==='court'||rpMode==='both'){
    scales.yCourt={position:'left',min:.5,max:nC+.5,ticks:{color:'#00e5ff',font:{size:10},stepSize:1,callback:v=>courtLetters[Math.round(v)]||''},grid:{color:'rgba(255,255,255,0.05)'},border:{color:'rgba(0,229,255,0.15)',dash:[3,3]}};}
  if(rpMode==='pts'||rpMode==='both'){
    const allPts=ptsData.filter(Boolean);const minP=Math.max(0,(Math.min(...allPts)||0)-2);const maxP=(Math.max(...allPts)||15)+2;
    scales.yPts={position:rpMode==='both'?'right':'left',min:minP,max:maxP,ticks:{color:'rgba(200,255,0,0.6)',font:{size:10}},grid:{drawOnChartArea:rpMode!=='both',color:'rgba(255,255,255,0.05)'},border:{color:'rgba(200,255,0,0.15)',dash:[3,3]}};}
  if(rpChart)rpChart.destroy();
  rpChart=new Chart(canvas,{type:'line',data:{labels,datasets},options:{responsive:true,maintainAspectRatio:false,interaction:{mode:'index',intersect:false},plugins:{legend:{display:false},tooltip:{backgroundColor:'#18181f',titleColor:'rgba(255,255,255,0.5)',titleFont:{size:10},bodyColor:'#f4f4f0',bodyFont:{size:11,weight:'700'},borderColor:'rgba(255,255,255,0.1)',borderWidth:1,padding:10,cornerRadius:8,callbacks:{title:items=>{const i=items[0].dataIndex;return 'Round '+(i+1)+(rounds[i].won?' · Won':' · Lost')},label:ctx=>{if(ctx.dataset.label==='Court'){return ' Court: '+(courtLetters[Math.round(ctx.raw)]||ctx.raw)}return ' Points: '+ctx.raw},labelColor:ctx=>{const i=ctx.dataIndex;const won=rounds[i].won;const c=ctx.dataset.label==='Court'?(won?'#00e5ff':'rgba(255,92,71,0.7)'):(won?'#c8ff00':'#ff5c47');return{borderColor:c,backgroundColor:c,borderRadius:3}}}}},scales,layout:{padding:{top:4,right:4}}}})}
...
function rStats(stats,season,l,ss){const has=stats.length>0&&stats.some(s=>s.w+s.l+s.t>0);let h='';const sessions=ss?[ss]:(season?season.sessions:[]);const isFinished=ss?ss.finished:false;
  ...
  // ── Round Progression chart card ──
  if(has&&stats.some(s=>s.roundRes.length>0)){
    h+='<div class="card fu">';
    h+='<h3 class="card-t">Round progression</h3>';
    h+='<div style="font-size:.72rem;color:var(--muted);margin-bottom:10px">Court position &amp; points each round</div>';
    // player chips
    const rpSorted=[...stats].filter(s=>s.roundRes.length>0).sort((a,b)=>b.pf-a.pf);
    const firstPid=rpPicked&&rpSorted.find(s=>s.id===rpPicked)?rpPicked:rpSorted[0]?.id;
    h+='<div class="rp-chips" id="rpChips">'+rpSorted.map(p=>'<button class="rp-chip'+(firstPid===p.id?' on':'')+'" onclick="rpSetPlayer(\''+p.id+'\')">'+p.name.split(' ')[0]+'</button>').join('')+'</div>';
    // mode buttons
    h+='<div class="rp-modes">';
    h+='<button id="rpBtn-both" class="rp-mbtn'+((!rpMode||rpMode==='both')?' active-both':'')+'" onclick="rpSetMode(\'both\')"><span class="rp-dot" style="background:#a78bfa"></span>Both</button>';
    h+='<button id="rpBtn-court" class="rp-mbtn'+(rpMode==='court'?' active-court':'')+'" onclick="rpSetMode(\'court\')"><span class="rp-dot" style="background:#00e5ff"></span>Court</button>';
    h+='<button id="rpBtn-pts" class="rp-mbtn'+(rpMode==='pts'?' active-pts':'')+'" onclick="rpSetMode(\'pts\')"><span class="rp-dot" style="background:#c8ff00"></span>Points</button>';
    h+='</div>';
    // legend
    h+='<div style="display:flex;gap:12px;margin-bottom:10px;font-size:.62rem;color:rgba(255,255,255,0.35)">';
    h+='<span><span style="display:inline-block;width:7px;height:7px;border-radius:50%;background:#00e5ff;margin-right:3px;vertical-align:middle"></span>Won</span>';
    h+='<span><span style="display:inline-block;width:7px;height:7px;border-radius:50%;background:#ff5c47;margin-right:3px;vertical-align:middle"></span>Lost</span>';
    h+='</div>';
    h+='<div style="position:relative;width:100%;height:200px"><canvas id="rpCanvas" role="img" aria-label="Round progression chart"></canvas></div>';
    h+='</div>';
  }
  ...
  else{const drSessions=ss?[ss]:(season?season.sessions:[]);const drRatings=calcDinkRating(stats,drSessions,l.players);h+='<div style="overflow-x:auto;margin:0 -18px;padding:0 18px"><table class="st"><thead><tr>'+['#','Player','W','L','Pts','+/-','Strk'].map(x=>'<th>'+x+'</th>').join('')+'<th class="dr-th">⬡ DR</th></tr></thead><tbody>';
    stats.filter(s=>s.w+s.l+s.t>0).forEach((s,i)=>{const d=s.pf-s.pa;const pn=pNum(s,l);const sk=s.streak;const skStr=sk>0?'W'+sk:sk<0?'L'+Math.abs(sk):'—';const dr=drRatings[s.id];const drStr=dr!=null?dr:'—';h+='<tr><td class="'+(i<3?'rt':'')+'">'+(["1st","2nd","3rd"][i]||(i+1))+'</td><td style="font-weight:600"><span style="font-family:\'Sora\',sans-serif;color:var(--lime);font-weight:700;font-size:.72rem;margin-right:4px">#'+pn+'</span>'+s.name+'</td><td class="at">'+s.w+'</td><td class="rdt">'+s.l+'</td><td>'+s.pf+'</td><td style="font-weight:700;color:'+(d>=0?'var(--lime)':'var(--loss)')+'">'+(d>0?'+':'')+d+'</td><td style="color:'+(sk>0?'var(--lime)':sk<0?'var(--loss)':'var(--muted)')+';font-weight:600">'+skStr+'</td><td class="dr-td"><span class="dr-val">'+drStr+'</span></td></tr>'});
    h+='</tbody></table></div>'}
  h+='</div>';return h}
...
function rOverview(l,s,stats){const as=s.sessions.filter(x=>!x.archived);let h='<div class="card fu"><div class="overline">Current season</div><h2 class="heading" style="font-size:1.2rem;color:var(--lime)">'+s.name+'</h2><div class="subtext" style="margin-top:4px">'+as.length+' ladder'+(as.length!==1?'s':'')+' · '+l.players.filter(p=>p.active!==false).length+' active players</div></div>';
  ...
  if(!as.length)h+='<p class="subtext" style="text-align:center;padding:20px">No ladders scheduled yet.</p>';
  else h+=[...as].reverse().map(x=>{const nParts=x.participants?x.participants.length:l.players.filter(p=>p.active!==false).length;const st=x.finished?'<span class="pill ok">Complete</span>':x.started?'<span class="pill live"><span class="dot"></span>Rd '+(x.currentRound+1)+'</span>':'<span class="pill draft">Upcoming</span>';return'<button class="sc" onclick="openSession(\''+x.id+'\')"><div style="display:flex;justify-content:space-between;align-items:center"><div><div style="font-weight:700;font-size:.9rem">'+(x.name||fmtDate(x.date))+'</div><div class="subtext" style="font-size:.72rem;margin-top:2px">'+fmtDate(x.date)+(x.config.startTime?' · '+fmt12(x.config.startTime):'')+' · '+nParts+' players · '+x.config.courts+' courts'+(x.config.place?' · '+x.config.place:'')+'</div></div>'+st+'</div></button>'}).join('');
  h+='</div>';return h}
...
function render(){
  ...
  if(tab==='stats'){setTimeout(tkRenderChart,10);setTimeout(rpRenderChart,20)}}
