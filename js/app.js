...
function rStats(stats,season,l,ss){const has=stats.length>0&&stats.some(s=>s.w+s.l+s.t>0);let h='';const sessions=ss?[ss]:(season?season.sessions:[]);const isFinished=ss?ss.finished:false;
  ...
  if(!has)h+='<p class="subtext" style="text-align:center;padding:20px">No scored games yet.</p>';
    ...
    stats.filter(s=>s.w+s.l+s.t>0).forEach((s,i)=>{const d=s.pf-s.pa;const pn=pNum(s,l);const sk=s.streak;const skStr=sk>0?'W'+sk:sk<0?'L'+Math.abs(sk):'—';h+='<tr><td class="'+(i<3?'rt':'')+'">'+(["1st","2nd","3rd"][i]||(i+1))+'</td><td style="font-weight:600"><span style="font-family:\'Sora\',sans-serif;color:var(--lime);font-weight:700;font-size:.72rem;margin-right:4px">#'+pn+'</span>'+s.name+'</td><td class="at">'+s.w+'</td><td class="rdt">'+s.l+'</td><td>'+s.pf+'</td><td style="font-weight:700;color:'+(d>=0?'var(--lime)':'var(--loss)')+'">'+(d>0?'+':'')+d+'</td><td style="color:'+(sk>0?'var(--lime)':sk<0?'var(--loss)':'var(--muted)')+';font-weight:600">'+skStr+'</td></tr>'});
    h+='</tbody></table></div>'}
  h+='</div>';return h}
